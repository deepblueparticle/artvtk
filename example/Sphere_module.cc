...
class artvtk::Sphere : public art::EDProducer {
public:
...
artvtk::Sphere::Sphere(fhicl::ParameterSet const & p) :
...
void artvtk::Sphere::produce(art::Event & e)
{
  ...
  std::unique_ptr<artvtk::VtkVizData> vizVtkData = std::make_unique<artvtk::VtkVizData>(mb, myName_);
...
vtkSmartPointer<vtkPolyData> artvtk::Sphere::makeSphereGrid()  {
...
vtkSmartPointer<vtkMultiBlockDataSet> artvtk::Sphere::makeMultiBlockForSphereGrid(vtkSmartPointer<vtkPolyData> grid) const {
...
DEFINE_ART_MODULE(artvtk::Sphere)
